...
async function apiRequest(url, installation, authorization) {
...
server.get("/api/*", async (request, reply) => {
    ...
    const data = await apiRequest(`api/v1${request.url.slice(4)}`, request.headers["installation"], request.headers["authorization"]);
    reply.send(data);
});
const start = async () => {
    try {
